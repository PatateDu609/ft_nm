...
void nm_32_symtab(t_file *file, Elf32_Shdr *sections, Elf32_Shdr *symtab, Elf32_Shdr *strtab)
{
	Elf32_Sym *sym;
	char *str;
	size_t i, j;
	...
	while (i < max)
	{
		if (add_symbol_x86(file->symbols + j++, str + sym[i].st_name,
...
void nm_32(t_file *file)
{
	...
	if (symtab && strtab)
		nm_32_symtab(file, shdr, symtab, strtab);
}
